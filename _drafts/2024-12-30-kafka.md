...
记得*修改 `KAFKA_CFG_ADVERTISED_LISTENERS` 的 ip 地址为自己的*
...
services:
  kafka:
    ...
    environment:
      ...
      - KAFKA_CFG_ADVERTISED_LISTENERS=PLAINTEXT://192.168.50.51:19092
      # 定义安全协议
...
命令的作用是更改 kafka 目录及其所有子目录和文件的所有者和所属组。

解释拆解：
sudo：以超级用户（root）权限运行命令。
chown：更改文件或目录的所有者。
-R：递归（Recursively）更改，应用于目录下的所有文件和子目录。
1001:1001：
1001（第一个）：新文件所有者的用户ID（UID）。
1001（第二个）：新文件所属组的组ID（GID）。
kafka：目标目录，表示将对 kafka 目录及其内容执行权限变更。
实际效果：
将 kafka 目录及其所有内容的所有者更改为 UID 为 1001 的用户，并将其所属组更改为 GID 为 1001 的组。
常用于 Docker 或服务部署，确保 Kafka 容器或进程可以访问和管理目录，避免权限问题。
示例场景：
Kafka 容器在运行时以 UID 1001 的用户身份运行，而宿主机上的 kafka 目录可能由其他用户拥有。
通过该命令，确保容器可以完全访问和修改 kafka 目录。
```
